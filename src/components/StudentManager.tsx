...
export default function StudentManager({ currentUser, classesList }: StudentManagerProps) {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} data siswa yang terpilih?`)) return;
    const path = 'students';
    try {
      setLoading(true);
      ...
      await sendRealtimeNotification(
        ...
        `${selectedIds.length} siswa telah dihapus dari kelas ${selectedClass}`,
        NotificationType.SISWA,
        selectedClass
      );
                ...
                {!isReadOnly && (
                  <th className="py-3 px-4 w-10 text-center">
                    <input
                      ...
                      checked={paginatedStudents.length > 0 && paginatedStudents.every(std => selectedIds.includes(std.id!))}
                      onChange={(e) => {
                        if (e.target.checked) {
                          const toAdd = paginatedStudents.map(std => std.id!).filter(id => !selectedIds.includes(id));
                          setSelectedIds([...selectedIds, ...toAdd]);
                        } else {
                          const toRemove = paginatedStudents.map(std => std.id!);
                  ...
                  {!isReadOnly && (
                    <td className="py-4 px-4 text-center">
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            setSelectedIds([...selectedIds, std.id!]);
                          } else {
                            setSelectedIds(selectedIds.filter(id => id !== std.id));
                          }
                        }}
                      />
                    </td>
